...
public:
	...
	*/
	void Update(float msec, float time);
	...
	*/
	...
	Vector3	SetPosition() { return Position; }

	/*
	Giving the center and the maximum variation of staring position for each axis
	*/
	void SetSnowingCube(Vector3 center, int dx, int dy, int dz);
...
protected:
	...
	*/
	Flake* GetFreeParticle(float time);
	...
	*/
	...
	std::vector<Flake*>	particles;	//Active particles stay in here :)
	std::vector<Flake*>	freeList;	//'Spare' particles stay in here...

										//Sets the speed that particles will move down;
	float Gravity;
};