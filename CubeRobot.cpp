...
CubeRobot::CubeRobot(void) {
	...
	hips = new SceneNode(cube, Vector4(0.8, 0.8, 0.0, 1.0));			//Yellow
	hips->SetModelScale(Vector3(10, -3.5, 5));
	hips->SetTransform(Matrix4::Translation(Vector3(0, 0, 0)));
	body->AddChild(hips);

	SceneNode* leftLeg = new  SceneNode(cube, Vector4(0, 0, 1, 1));   //Blue!
	leftLeg->SetModelScale(Vector3(3, -17.5, 3));
	leftLeg->SetTransform(Matrix4::Translation(Vector3(-7.5, -6, 0)));
	body->AddChild(leftLeg);

	SceneNode* rightLeg = new  SceneNode(cube, Vector4(0, 0, 1, 1)); //Blue!
	rightLeg->SetModelScale(Vector3(3, -17.5, 3));
	rightLeg->SetTransform(Matrix4::Translation(Vector3(7.5, -6, 0)));
...
void  CubeRobot::Update(float  msec, bool moving) {

	if (moving) {
		leftArm->SetTransform(leftArm->GetTransform()*Matrix4::Rotation(-10.0f, Vector3(1, 0, 0)));
		rightArm->SetTransform(rightArm->GetTransform()*Matrix4::Rotation(10.0f, Vector3(1, 0, 0)));
