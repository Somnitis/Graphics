...
*/
ParticleEmitter::ParticleEmitter(Vector3 x) {
	SetPosition(x);
	particleRate = 5.0f;
	...
	*/
	...
	texture = SOIL_load_OGL_texture(TEXTUREDIR"red.png",
		SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_COMPRESS_TO_DXT);
...
*/
void ParticleEmitter::Update(float msec, float time) {
...
*/
Particle* ParticleEmitter::GetFreeParticle(float time) {
	Particle * p = NULL;
	...
	p->colour = Vector4(1, 0, 0, 1.0);
	p->direction = initialDirection;
	p->direction.x += ((RAND() - RAND()) * particleVariance);
	p->direction.y += 6 + ((2 * RAND()/*- Gravity*sin(time/2) / 1*//*-RAND()*/) * particleVariance);
